    ...
    public function CrearVistaOrdenTrabajo($db) {
        ...
        $sql="CREATE VIEW vista_ordenes_trabajo AS
                SELECT t1.*,
                    ...
                    (SELECT (SELECT t1.horas_programadas)-horas_trabajo FROM equipos_componentes t7 WHERE t7.ID=t1.componente_id LIMIT 1) AS diferencia_horas,
                    (SELECT (SELECT t1.kilometros_programados)-kilometros_trabajo FROM equipos_componentes t7 WHERE t7.ID=t1.componente_id LIMIT 1) AS diferencia_kilometros,
